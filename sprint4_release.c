...
typedef struct {
	...
	//Booleen taches_affectees_par_specialites[MAX_SPECIALITES]; //-> 1 travailleur peut-être associé à plusieurs spé ; 1 spé qu'à 1 travailleur; tableau propre à chaque travailleur FAUX
} Travailleurs;
...
#define MAX_TRAVAILLEURS_PAR_TACHE 10;

typedef struct {
	unsigned int nb_heures_requises;
	unsigned int nb_heures_effectuees;
	unsigned int id_travailleur; // -> ? travailleurs par tache ; 1 tache par travailleur => mieux pour lire avec charge()
}Tache;
...
void traite_charge(Specialites* ptr_specialites, Travailleurs* ptr_travailleurs, Commandes* ptr_commandes) {
	...
	for (int i = 0; i < ptr_commandes->nb_commandes; ++i) {
		...
		{
			if (ptr_commandes->tab_commandes[i].tache_par_specialite[j].id_travailleur == indice_travailleur) {
				if (virgule == VRAI) {
					printf(", ");
				}
